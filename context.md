...
**Postman коллекция:** 33 теста, все проходят (тест 3.7 удалён — см. Д-030)

> **Архитектурное изменение [Д-031]:** endpoint переименован `/beneficiaries` → `/beneficiaries/validate`, статус успеха `202` → `200 { message: "Ok" }`, `request_id` упразднён. Postman-коллекция требует обновления.
...
| `VALIDATION_WARNING` | 202 | warnings[] |
| `COMPLIANCE_ESCALATION` | 202 | Только в audit log |
...
| Д-030 | COMPLIANCE_ESCALATION по FATCA недостижима — gap в правилах | Аналитик + Комплаенс |
