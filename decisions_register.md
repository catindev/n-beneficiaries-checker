...
**Дата:** 28.02.2026 | **Версия:** 1.3 | **Статус:** Draft
...
| Д-006 | HTTP-статусы: `400` — отсутствует заголовок `X-Merchant-Id`, `403` — комплаенс-триггер, `422` — ошибки валидации, `202` — успех                                                                                                                                                                     | Договорённость с командой                   | —                                                                      | `Закрыт` |
...
| Д-031 | `n-beneficiaries-checker` — внутренний синхронный сервис-валидатор. Клиент — оркестратор, не мерчант напрямую. Статус успешного ответа изменён с `202` на `200 { "message": "Ok" }`. `request_id` в ответе упразднён. Сервис не управляет заявками — только валидирует payload и возвращает вердикт | Архитектурное решение — пересмотр концепции | Обновить код `index.js`: статус 202→200, убрать генерацию `request_id` | `Открыт` |
...
| Д-013 | `VALIDATION_WARNING` включён в прототип для мягких контролей. Не блокирует заявку. Возвращается в отдельном массиве `warnings`. При успехе — `202 + warnings`                                                                                                                                                             | Договорённость с командой                                                                 | После прототипа вынести на оценку бизнесу и комплаенсу — несут ли мягкие контроли регуляторный риск                                                        | `Открыт` |
...
| Д-030 | Gap в правилах — `COMPLIANCE_ESCALATION` по `FATCA_FLAG_CONTRADICTION` недостижима: любое условие срабатывания эскалации (`birth_place` содержит "США") одновременно является основанием для `COMPLIANCE_BLOCK` через правило `usa_birth_place`. Сценарий эскалации без блокировки по FATCA в текущих правилах невозможен | Выявлено при тестировании коллекции Postman                                               | Уточнить у бизнеса — существуют ли иные косвенные признаки FATCA которые должны давать эскалацию без блокировки. До уточнения тест 3.7 из коллекции удалён | `Открыт` |
...
| ID     | Решение                                                                                                                                                    | Компонент            | Примечание                                                                |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------- |
| Д-013т | Четыре типа событий: `COMPLIANCE_BLOCK` → 403, `VALIDATION_ERROR` → 422, `VALIDATION_WARNING` → 202 + warnings, `COMPLIANCE_ESCALATION` → только audit log | `context-builder.js` | Приоритет: `COMPLIANCE_BLOCK` > `VALIDATION_ERROR` > `VALIDATION_WARNING` |
...
| Д-030 | Уточнить у бизнеса существуют ли косвенные признаки FATCA дающие эскалацию без блокировки                | Аналитик + Комплаенс | До написания ТЗ на разработку |
| Д-031 | Обновить `index.js`: статус 202→200, убрать генерацию `request_id`                                       | Разработка           | Ближайший спринт              |
